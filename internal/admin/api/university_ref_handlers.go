...
func createEntity[T any](h *UniversityRefHandler, w http.ResponseWriter, r *http.Request, query string, conflictMsg string, args func(T) []any) {
	var req T
	...
	var id int64
	if err := h.pool.QueryRow(r.Context(), query, args(req)...).Scan(&id); err != nil {
...
func updateEntity[T any](h *UniversityRefHandler, w http.ResponseWriter, r *http.Request, query string, conflictMsg string, args func(int64, T) []any) {
	id, err := h.pathID(r)
	if err != nil {
	...
	if _, err := h.pool.Exec(r.Context(), query, args(id, req)...); err != nil {
		writeError(w, http.StatusConflict, conflictMsg)
		return
	}
...
func (h *UniversityRefHandler) handleCreateFaculty(w http.ResponseWriter, r *http.Request) {
	createEntity[facultyRequest](h, w, r,
		`INSERT INTO faculties (code, name, short_name) VALUES ($1, $2, NULLIF($3,'')) RETURNING id`,
		"failed to create: code may already exist",
		func(req facultyRequest) []any {
			return []any{req.Code, req.Name, req.ShortName}
		},
	)
}

func (h *UniversityRefHandler) handleUpdateFaculty(w http.ResponseWriter, r *http.Request) {
	updateEntity[facultyRequest](h, w, r,
		`UPDATE faculties SET code=$2, name=$3, short_name=NULLIF($4,''), updated_at=now() WHERE id=$1`,
		"failed to update",
		func(id int64, req facultyRequest) []any {
			return []any{id, req.Code, req.Name, req.ShortName}
		},
	)
}
...
func (h *UniversityRefHandler) handleCreateSemester(w http.ResponseWriter, r *http.Request) {
	createEntity[semesterRequest](h, w, r,
		`INSERT INTO semesters (year, semester_type) VALUES ($1, $2) RETURNING id`,
		"failed to create: semester may already exist",
		func(req semesterRequest) []any {
			return []any{req.Year, req.SemesterType}
		},
	)
}

func (h *UniversityRefHandler) handleUpdateSemester(w http.ResponseWriter, r *http.Request) {
	updateEntity[semesterRequest](h, w, r,
		`UPDATE semesters SET year=$2, semester_type=$3, updated_at=now() WHERE id=$1`,
		"failed to update",
		func(id int64, req semesterRequest) []any {
			return []any{id, req.Year, req.SemesterType}
		},
	)
}
