...
- **`reconfigure`** - используется при изменении конфига, если плагин реализует `OnReconfigure`. Для legacy-плагина без `OnReconfigure` хост делает controlled reload того же wasm-бинаря.
